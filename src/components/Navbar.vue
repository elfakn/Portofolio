...
<template>
  <header class="fixed top-0 left-0 w-full z-50 transition-all duration-300 border-b" :class="isScroll
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 py-3.5 flex items-center justify-between gap-4">
      <a href="#top"
        ...
        aria-label="Go to homepage" @click="active = 'top'">
        <span class="inline-flex items-center gap-2">
          <span
            class="inline-flex h-7 w-7 items-center justify-center rounded-lg bg-brand-600 text-white text-xs font-extrabold">EK</span>
          Elfa Khoirun Nissa
        </span>
      </a>

      <nav class="hidden md:flex items-center gap-1" aria-label="Primary">
        <a v-for="l in links" :key="l.id" :href="l.href" :aria-current="active === l.id ? 'page' : undefined"
          class="px-3 py-2 rounded-full text-sm font-medium transition-colors focus-visible:outline-none" :class="active === l.id
        ...
        <button :aria-label="isDark ? 'Switch to light theme' : 'Switch to dark theme'"
      ...
      <div class="flex md:hidden items-center gap-2">
        ...
        <button aria-label="Open menu" aria-expanded="false" aria-controls="mobile-drawer"
    ...
    <div v-show="isOpen" class="fixed inset-0 bg-gray-900/40 backdrop-blur-sm md:hidden z-40" aria-hidden="true"
      @click="closeMenu" />

    <aside id="mobile-drawer" role="dialog" aria-modal="true" aria-label="Navigation menu"
      ...
      <nav class="flex flex-col p-3 gap-1" aria-label="Mobile">
        <a v-for="l in links" :key="l.id" :href="l.href" class="px-4 py-3 rounded-xl text-[15px] font-medium transition"
          :class="active === l.id
              ? 'bg-gray-900 text-white dark:bg-white dark:text-gray-900'
        ...
        <p class="px-4 pt-6 text-xs text-gray-400 dark:text-gray-500">
          © {{ new Date().getFullYear() }} Elfa Khoirun Nissa
        </p>
      </nav>
    </aside>
  </header>
