...
<template>
  <section id="experience" class="w-11/12 max-w-6xl mx-auto py-16 sm:py-20">
    <div class="text-center mb-8">
      ...
      <h2
        class="mt-2 font-display text-3xl font-extrabold tracking-tight text-gray-900 dark:text-white"
      >
        Experience & Projects
      </h2>
      <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
        Real-world delivery across internal systems and personal builds
      </p>
    </div>

    <div class="flex justify-center mb-8" role="tablist" aria-label="Experience and projects">
      <div
        class="inline-flex items-center gap-1 rounded-full border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 p-1.5 shadow-sm"
      >
        <button
          type="button"
          role="tab"
    ...
    <div
      ...
      :key="'exp'"
      class="grid md:grid-cols-2 gap-4 sm:gap-5"
      role="tabpanel"
    >
      <article
      ...
      >
        <div class="flex items-start justify-between gap-3">
          ...
          <span
            class="shrink-0 rounded-full bg-emerald-100 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-300 px-2.5 py-1 text-xs font-semibold"
            >{{ exp.year }}</span
          >
        </div>
        ...
        <ul class="mt-4 space-y-2 text-sm leading-6 text-gray-600 dark:text-gray-300">
          <li v-for="(item, i) in exp.description" :key="i" class="flex gap-2.5">
            <span
              aria-hidden="true"
              class="mt-[9px] h-1.5 w-1.5 shrink-0 rounded-full bg-brand-500"
            ></span>
            <span>{{ item }}</span>
          </li>
        </ul>
    ...
    <div v-else :key="'proj'" class="grid sm:grid-cols-2 gap-4 sm:gap-5" role="tabpanel">
      <article
      ...
      >
        <div
          class="relative bg-gradient-to-br from-gray-100 via-gray-50 to-white dark:from-white/10 dark:via-white/5 dark:to-transparent"
        >
          <img
            :src="project.image"
          ...
          />
          <span
            class="absolute left-4 top-4 rounded-full bg-white/90 dark:bg-gray-950/70 backdrop-blur px-2.5 py-1 text-[11px] font-semibold text-gray-700 dark:text-gray-200 border border-gray-200 dark:border-white/10"
            >{{ index === 0 ? 'Featured' : 'Personal' }}</span
          >
        </div>
        <div class="p-5 sm:p-6">
          <h3 class="font-display text-lg font-bold tracking-tight text-gray-900 dark:text-white">
            {{ project.title }}
          </h3>
